...
export default function Home() {
  ...
  const getFromCloudStorage = async (key: string): Promise<any> => {
    try {
      const WebApp = (await import('@twa-dev/sdk')).default;
      const result = await WebApp.storage.get(key);
      return result[key];
    } catch (error) {
      console.error('Error retrieving from cloud storage:', error);
      return undefined;
    }
  };
  ...
  const handleTapToEarn = () => {
    ...
    // Save updated token count to cloud storage
    saveToCloudStorage('tokenCount', newTokenCount);
  };
